<template>
  <div id="article" class="main-container">
    <header class="main-header article-header">
      ...
      >
        ...
        <el-menu-item index="1">精选文章</el-menu-item>
        <el-menu-item index="2">平台文章</el-menu-item>
        <el-menu-item index="3">用户文章</el-menu-item>
      </el-menu>
    </header>
    <div class="main-tool">
      <div>
    ...
    <div class="table-data">
      ...
      >
        ...
        <el-table-column prop="articleContainer" label="文章概要" width="434px">
          <template slot-scope="scope">
            <div class="text-container">
              <!-- <div
                :style="{backgroundImage: 'url('+ scope.row.picture+')',backgroundRepeat: 'no-repeat',backgroundPosition: '100% 100%',backgroundSize: 'cover'}"
              ></div>-->
              <div class="text-picture">
                <img :src="scope.row.picture?scope.row.picture:require('@/assets/article.svg')" />
              </div>
              <div class="text-info">
        ...
        <el-table-column
          ...
          width="120px"
        ></el-table-column>
        <el-table-column v-if="type==='user'" prop="publish" label="谁可见" width="120px"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" width="250px">
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="changeSelect(true,scope.row._key,2)"
              type="text"
              v-if="type!=='selectReview'&&!scope.row.block"
            >设为屏蔽</el-button>
            <el-button
              @click.native.prevent="changeSelect(false,scope.row._key,2)"
              type="text"
              v-if="type!=='selectReview'&&scope.row.block"
            >取消屏蔽</el-button>
            <el-button
              @click.native.prevent="changeSelect(true,scope.row._key,3)"
              type="text"
              v-if="type==='selected'&&!scope.row.top"
            >设为置顶</el-button>
            <el-button
              @click.native.prevent="changeSelect(false,scope.row._key,3)"
              type="text"
              v-if="type==='selected'&&scope.row.top"
            >取消置顶</el-button>
            <el-button
    ...
    <ViewArticleDialog v-if="viewShowState" :targetArticleKey="targetArticleKey" />
  </div>
</template>
...
import ViewArticleDialog from "./viewArticleDialog/viewArticleDialog";
export default {
  name: "Article",
  components: {
    ReviewArticleDialog,
  ...
  mounted: async function() {
    ...
    this.$store.dispatch("changeReviewState", false);
    this.getData();
  },
  computed: {
    ...mapState({
  ...
  watch: {
    reviewState(val, oldVal) {
      //普通的watch监听
      if (oldVal !== val && oldVal) {
        this.getData();
      }
    },
    reviewShowState(val, oldVal) {
      //普通的watch监听
      if (oldVal !== val && oldVal) {
        this.getData();
      }
    },
    viewShowState(val, oldVal) {
      //普通的watch监听
      if (oldVal !== val && oldVal) {
        this.getData();
      }
